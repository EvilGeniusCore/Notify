# Notify.Teams — Developer Guide
> For .NET developers who want to send Teams messages directly from their application without shelling out to the CLI.

## Overview

`Notify.Teams` is a .NET library that wraps the Microsoft Graph API for sending messages to Microsoft Teams channels. It handles authentication via Client Credentials (app-only), team and channel resolution by name or GUID, and message delivery.

Use this library if:
- You are building a .NET application that needs to send Teams notifications programmatically
- You want typed, awaitable API calls rather than shelling out to the `notify` CLI
...
{
    TenantId     = Environment.GetEnvironmentVariable("NOTIFY_TEAMS_TENANT_ID")
                   ?? throw new InvalidOperationException("NOTIFY_TEAMS_TENANT_ID not set"),
    ClientId     = Environment.GetEnvironmentVariable("NOTIFY_TEAMS_CLIENT_ID")
                   ?? throw new InvalidOperationException("NOTIFY_TEAMS_CLIENT_ID not set"),
    ClientSecret = Environment.GetEnvironmentVariable("NOTIFY_TEAMS_CLIENT_SECRET")
                   ?? throw new InvalidOperationException("NOTIFY_TEAMS_CLIENT_SECRET not set"),
};
