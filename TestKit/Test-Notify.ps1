...
<#
.SYNOPSIS
    Smoke-tests notify against a real Teams environment.
...
$binary  = Join-Path $root 'notify.exe'
...
if (-not (Test-Path $binary)) {
    Write-Error "notify.exe not found in this folder. Download the win-x64 release zip from the releases page, extract notify.exe here, and re-run."
}
...
Write-Host "notify smoke test" -ForegroundColor White
...
Run-Test "2. Send dry-run (team/channel resolution)" {
    & $binary send --message "notify dry-run test" --dry-run
}
...
Run-Test "3. Send real message" {
    & $binary send --message "notify smoke test — $hostname — $timestamp"
}
