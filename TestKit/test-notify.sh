...
if [[ ! -f "$BINARY" ]]; then
    echo "ERROR: notify binary not found in this folder." >&2
    echo "Download the release zip for your platform from the releases page," >&2
    echo "extract the binary here, run 'chmod +x notify', then re-run this script." >&2
    exit 1
fi

if [[ ! -x "$BINARY" ]]; then
    echo "ERROR: notify is not executable. Run: chmod +x notify" >&2
    exit 1
fi
...
run_test "3. Send real message" \
    "$BINARY" send --message "notify smoke test — $HOSTNAME_VAL — $TIMESTAMP"
