...
public static class SendCommand
{
    public static Command Build(Option<string?> envFileOption)
    {
        ...
        var teamOption    = new Option<string?>(["-t", "--team"],     "Target team name or GUID (overrides TEAMS_NOTIFY_DEFAULT_TEAM)")
            { ArgumentHelpName = "name|id" };
        var channelOption = new Option<string?>(["-c", "--channel"],  "Target channel name or GUID (overrides TEAMS_NOTIFY_DEFAULT_CHANNEL)")
            { ArgumentHelpName = "name|id" };
        ...
        command.SetHandler(async (InvocationContext context) =>
        {
            ...
            try
            {
                ...
                {
                    Console.Error.WriteLine("error: no team specified — use --team or set TEAMS_NOTIFY_DEFAULT_TEAM.");
                ...
                {
                    Console.Error.WriteLine("error: no channel specified — use --channel or set TEAMS_NOTIFY_DEFAULT_CHANNEL.");
                    context.ExitCode = ExitCodes.ConfigurationMissing;
                    return;
                }
