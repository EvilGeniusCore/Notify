...
Source code and full documentation are on GitHub at [Bonejob/teams-notify](https://github.com/Bonejob/teams-notify).

If you want to see a complete working implementation using this library, the `teams-notify` CLI tool in that repository is built entirely on `TeamsNotify.Core` and covers credential loading, command parsing, exit code handling, and dry-run support — a useful reference for integrating the library into your own application.
